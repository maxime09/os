...
use crate::{
    PTE_PRESENT, PTE_READ_WRITE, alloc_page_phys_addr, keyboard, map_page_kernel, rsdt::MADT,
};
pub mod timer;
...
pub fn get_APIC_BASE() -> u64 {
    ...
    unsafe { msr.read() & 0xfffffffffffff000 }
}

pub fn is_bsp() -> bool {
    let msr = Msr::new(IA32_APIC_BASE_MSR);
    unsafe { (msr.read() & IA32_APIC_BASE_MSR_BSP) != 0 }
}
...
pub fn write_lapic(reg: u32, value: u32) {
    unsafe {
        ...
        (base_addr as *mut u32).byte_offset(0x20).read_volatile(); // read to ensure we wait long enough for the write to complete
    }
}
...
pub fn setup_interrupt_redirection(
...
pub fn setup_keyboard_interrupt(madt: &MADT) {
    x86_64::instructions::interrupts::without_interrupts(|| {
        ...
        setup_interrupt_redirection(index, 0x31, 0x00, false, false, false, false, 0);
        keyboard::reset();
        keyboard::init();
    });
}
...
pub fn setup_PIT_interrupt(madt: &MADT) {
    ...
    setup_interrupt_redirection(index, 0x30, 0x00, false, false, false, false, 0);
}

pub unsafe fn get_io_apic_version() -> u8 {
    unsafe { (read_io_apic(1) & 0xff) as u8 }
}
